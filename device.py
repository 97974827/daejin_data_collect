...
20. 리더기 설정 불러오기       # get_reader_config
21. 리더기 설정                # set_reader_config
"""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""


class Device:
    # 485 스레드 제어 변수
    USE = False                 # True 일때 외부에서 실시간 스레드 종료할 수 있음
    USE_EACH = True             # 실시간 모니터링에서 True 일때 실시간 스레드 작동

    TIME_USE = False            # True 일때 외부에서 종료 할 수 있음
    TIME_USE_EACH = True        # 시간설정에서 True 일때 시간설정 함

    FLAG_MAIN = ''              # 상태 : thread, cancel(장비셋팅), monitor(지금은 쓰지않음)
    FLAG_STATE = ''             # 상태 : start, stop(장비셋팅, 부가기능, 실시간 모니터링 )

    # 스레드 상수화
    device_state_thread = ''    # 실시간 모니터링 스레드 변수
    state_monitor = ''          # 현재 안쓰는 변수
    ...
    def main_thread(self, second=1):
        try:
            ...
            # print("*** main_thread 디버깅")
            # print("USE           : ", self.USE)
            # print("USE_EACH      : ", self.USE_EACH)
            # print("TIME_USE      : ", self.TIME_USE)
            # print("TIME_USE_EACH : ", self.TIME_USE_EACH)
        ...
        try:
            with conn.cursor():
                ...
                curs.execute(get_device_qry,
                             (gls_config.SELF, gls_config.AIR, gls_config.MATE, gls_config.CHARGER, gls_config.READER,
                              gls_config.GARAGE))
                ...
                for device in device_list:
                    if self.USE_EACH:
                        ...
                        if (device['type'] == gls_config.SELF or device['type'] == gls_config.AIR
                                or device['type'] == gls_config.MATE or device['type'] == gls_config.READER or device[
                                    'type'] == gls_config.GARAGE):
                            ...
                            try:
                                ...
                                if stx == 'GL':
                                    ...
                                    if (state[5:7] == 'VR' or state[5:7] == 'MR' or state[5:7] == 'RR') and state[61:63] == 'CH' and state_len == 63:
                                        ...
                                        if check_sum == res_check_sum:
                                            temp_state['state'] = '1'
                                            temp_state['start_time'] = str(state[11:13]) + ":" + str(
                                                state[13:15]) + ":" + str(state[15:17])
                                    ...
                                    if state[5:7] == 'SW' and state[75:77] == 'CH' and state_len == 77:
                                        ...
                                        if check_sum == res_check_sum:
                                            ...
                                            if t_year != year or int(month) == 0 or int(month) > 12 or int(day) == 0 or int(day) > 31 or e_m != t_minute:
                                                ...
                                                use_second = int(self_time) + int(foam_time) + int(under_time) + int(
                                                    coating_time)
                                    ...
                                    if state[5:7] == 'VW' and state[59:61] == 'CH' and state_len == 61:
                                        ...
                                        if check_sum == res_check_sum:
                                            ...
                                            t_minute = datetime.today().strftime('%M')
                                            t_second = datetime.today().strftime('%S')
                                            if t_year != year or int(month) == 0 or int(month) > 12 or int(
                                                    day) == 0 or int(day) > 31 or e_m != t_minute:
                                    ...
                                    if state[5:7] == 'GW' and state[89:91] == 'CH' and state_len == 91:
                                        ...
                                        if check_sum == res_check_sum:
                                            ...
                                            curs.execute(garage_insert, (
                                            device['addr'], card_num, remain_card, card_use, cash_use, master_use,
                                    ...
                                    if (state[5:7] == 'SN' or state[5:7] == 'GN') and state[
                                                                                      39:41] == 'CH' and state_len == 41:
                                        ...
                                        if check_sum == res_check_sum:
                                            ...
                                            curs.execute(t_update_qry,
                                                         (cash, card, master, version, device['type'], device['addr']))
                                            conn.commit()

                                    # 진공 / 매트 / 리더(매트) 저장값 없는 대기 동작
                                    if (state[5:7] == 'VN' or state[5:7] == 'MN' or state[5:7] == 'RN') and state[
                                                                                                            38:40] == 'CH' and state_len == 40:
                                ...
                                print("From get_device_state for 485 : ", e) # 에러로그 발생하는데 발생한 장비부터 그 뒤장비까지 전부 쓰레기값 표시됨
                                ...
                                if stx == 'GL' and (state_len == 61 or state_len == 57):
                                    ...
                                    if check_sum == res_check_sum:
                                        ...
                                        if state[5:7] == 'CW' and etx == 'CH':
                                            ...
                                            if today_year != year or int(month) == 0 or int(month) > 12 or int(
                                                    day) == 0 or int(day) > 31:
                            ...
                            if self_config['state'] != '0':
                                ...
                                if diff == '1':
                                    ...
                                    curs.execute(new_self_config_qry, (self_config['device_addr'],
                                                                       str(self_config['self_init_money'] // 100).rjust(
                                                                           3, '0'),
                                                                       self_config['self_init_time'],
                                                                       self_config['self_con_enable'],
                                                                       str(self_config['self_con_money'] // 100).rjust(
                                                                           3, '0'),
                                                                       ...
                                                                       str(self_config['foam_init_money'] // 100).rjust(
                                                                           3, '0'),
                                                                       self_config['foam_init_time'],
                                                                       str(self_config['foam_con_money'] // 100).rjust(
                                                                           3, '0'),
                                                                       ...
                                                                       str(self_config[
                                                                               'under_init_money'] // 100).rjust(3,
                                                                                                                 '0'),
                                                                       self_config['under_init_time'],
                                                                       str(self_config['under_con_money'] // 100).rjust(
                                                                           3, '0'),
                                                                       ...
                                                                       str(self_config[
                                                                               'coating_init_money'] // 100).rjust(3,
                                                                                                                   '0'),
                                                                       self_config['coating_init_time'],
                                                                       str(self_config[
                                                                               'coating_con_money'] // 100).rjust(3,
                                                                                                                  '0'),
                                                                       self_config['coating_con_time'],
                                                                       self_config['coating_pause_time'],
                                                                       str(self_config['cycle_money'] // 100).rjust(3,
                                                                                                                    '0'),
        ...
        try:
            ...
            with conn.cursor():
                ...
                curs.execute(insert_self_config_q, (device_addr, self_init_money, self_init_time, self_con_enable,
                                                    ...
                                                    pay_free, buzzer_time, pause_count, set_coating_output,
                                                    secret_enable,
                            ...
                            if air_config['state'] != '0':
                                ...
                                if diff == '1':
                                    ...
                                    curs.execute(new_air_config_qry, (air_config['device_addr'],
                                                                      str(air_config['air_init_money'] // 100).rjust(3,
                                                                                                                     '0'),
                                                                      air_config['air_init_time'],
                                                                      str(air_config['air_con_money'] // 100).rjust(3,
                                                                                                                    '0'),
                                                                      air_config['air_con_time'],
                                                                      str(air_config['cycle_money'] // 100).rjust(3,
                                                                                                                  '0'),
                            ...
                            if mate_config['state'] != '0':
                                ...
                                if diff == '1':
                                    ...
                                    curs.execute(new_mate_config_qry, (mate_config['device_addr'],
                                                                       str(mate_config['mate_init_money'] // 100).rjust(
                                                                           3, '0'),
                                                                       mate_config['mate_init_time'],
                                                                       str(mate_config['mate_con_money'] // 100).rjust(
                                                                           3, '0'),
                                                                       mate_config['mate_con_time'],
                                                                       str(mate_config['cycle_money'] // 100).rjust(3,
                                                                                                                    '0'),
                        ...
                        for db_row in db_config:
                            ...
                            if diff == '1':
                                ...
                                curs.execute(update_config_qry, (device_no,
                                                                 str(int(charger_config['card_price']) // 100).rjust(3,
                                                                                                                     '0'),
                                                                 str(int(
                                                                     charger_config['card_min_price']) // 100).rjust(3,
                                                                                                                     '0'),
                                                                 str(int(
                                                                     charger_config['auto_charge_price']) // 100).rjust(
                                                                     3, '0'),
                            ...
                            if diff == '2':
                                ...
                                curs.execute(update_bonus_qry, (str(int(charger_config['bonus1']) // 100).rjust(3, '0'),
                                                                ...
                                                                str(int(charger_config['bonus10']) // 100).rjust(3,
                                                                                                                 '0'),
    ...
    * 참고 : 본 함수는 포스 설정에서 이미 변경되어 있는 상점 ID를 장비에 입력하는 기능
            ...
            try:
                with conn.cursor():
                    ...
                    for device in get_device_no_res:
                        ...
                        conn.commit()
                        # 터치 충전기 업데이트
                        pi_conn = pymysql.connect(host=device['ip'], user=gls_config.PI_MYSQL_USER,
                                                  password=gls_config.PI_MYSQL_PWD,
    ...
    # noinspection PyMethodMayBeStatic
    # 포스 - 메인화면 으로 돌아올때마다 호출
    # start_thread -> thread_monitor
                            ...
                            if garage_config['state'] != '0':
                                ...
                                if diff == '1':
                                    ...
                                    curs.execute(new_garage_config_qry, (garage_config['device_addr'],
                                                                         str(garage_config[
                                                                                 'btn1_init_money'] // 100).rjust(3,
                                                                                                                  '0'),
                                                                         garage_config['btn1_init_time'],
                                                                         str(garage_config[
                                                                                 'btn2_init_money'] // 100).rjust(3,
                                                                                                                  '0'),
                                                                         garage_config['btn2_init_time'],
                                                                         str(garage_config['con_money'] // 100).rjust(3,
                                                                                                                      '0'),
                                                                         garage_config['con_time'],
                                                                         garage_config['self_time'],
                                                                         garage_config['self_count'],
                                                                         garage_config['foam_time'],
                                                                         garage_config['foam_count'],
                                                                         garage_config['under_time'],
                                                                         garage_config['under_count'],
                                                                         garage_config['coating_time'],
                                                                         garage_config['coating_count'],
                                                                         garage_config['air_time'],
                                                                         garage_config['air_count'],
                                                                         garage_config['airgun_time'],
                                                                         garage_config['airgun_count'],
                                                                         garage_config['buzzer_time'],
                                                                         garage_config['coating_type'],
                                                                         garage_config['cycle_money'],
                                                                         insert_input_date))
                                    conn.commit()
            finally:
                conn.close()
        ...
        try:
            ...
            with conn.cursor():
                ...
                curs.execute(new_garage_config_qry, (device_addr,
                                                     ...
                                                     con_money, con_time,
                                                     self_time, self_count, foam_time, foam_count, under_time,
                                                     under_count,
                                                     coating_time, coating_count, air_time, air_count, airgun_time,
                                                     airgun_count,
                    ...
                    for row in addr_res:
                        ...
                        try:
                            ...
                            if stx == 'GL' and etx == 'CH' and res_len == 33 and res_check_sum == check_sum:
                                ...
                                #reader_config['check_sum'] = res[29:31]
                            else:
                        ...
                        for db_row in db_config:
                            ...
                            db_reader_config['reader_con_enable'] = int(db_row['reader_con_enable'])
                            db_reader_config['reader_init_pulse'] = int(db_row['reader_init_pulse'])
                            db_reader_config['reader_con_pulse'] = int(db_row['reader_con_pulse'])
                            db_reader_config['reader_pulse_duty'] = int(db_row['reader_pulse_duty'])
                            ...
                            if reader_config['state'] != '0':
                                ...
                                if db_reader_config['reader_cycle_money'] != reader_config['reader_cycle_money']:
                                    print("db cycle money: ", db_reader_config['reader_cycle_money'])
                                    print("cycle money: ", reader_config['reader_cycle_money'])
                                    diff = '1'
                                    diff_set['state'] = '2'
                                    diff_set['device_addr'] = row['addr']
                                ...
                                if diff == '1':
                                    ...
                                    curs.execute(new_reader_config_qry, (reader_config['device_addr'],
                                                                         ...
                                                                         str(reader_config['reader_init_pulse']).rjust(2,'0'),
                                                                         str(reader_config['reader_con_pulse']).rjust(2,'0'),
                                                                         str(reader_config['reader_pulse_duty']).rjust(4,'0'),
                                                                         insert_input_date))
                                    conn.commit()
            finally:
                conn.close()
    ...
    def set_reader_config(self, args):
        ...
        try:
            ...
            with conn.cursor():
                ...
                print("set_reader_config : ", trans)

                try:
                    res = ser.readline(ser.write(bytes(trans)))
