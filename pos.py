...
class Pos:
    ...
    * 현재 auth_code만 구현 되어 있음
            ...
            with conn.cursor():
                if str(type) == 'self':
                    ...
                    if self.DELETE_STATE:
                        query = "SELECT `self_time`, `under_time`, `foam_time`, `coating_time` FROM gl_delete_self_state AS self WHERE self.`no` = %s"
                    curs.execute(query, no)
    ...
    # TODO : 20/05.22 대진 업데이트 내용 - 장비 세부 이력 조회 : 삭제(휴지통) 기능 (셀프, 리더기, 충전기만 만듬)
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    50. 장비 세부이력 - 삭제 데이터 조회
    포스로부터 장비 타입과 장비 주소를 인자로 넘겨받고, 해당 장비의 삭제 데이터 이력을 
    반환함.
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def get_delete_data_select(self, type, addr):
        # print(type)
        # print(addr)

        dic_delete_data_select = []  # 반환값
        self.DELETE_STATE = True  # 삭제 조회 플래그 ON
        ...
        try:
            with conn.cursor():
                # 세차장비 (사용내역, 삭제일, 시작일, 종료일, 현금/카드사용액, 카드 잔액, 카드 번호)
                if (type == str(gls_config.SELF) or type == str(gls_config.AIR) or type == str(gls_config.MATE)
                        or type == str(gls_config.READER) or type == str(gls_config.GARAGE)):
                    if type == str(gls_config.SELF):
                        query = "SELECT delete_self.`no`, UNIX_TIMESTAMP(`start_time`) AS 'start_time', " \
                                "UNIX_TIMESTAMP(`end_time`) AS 'end_time', " \
                                "UNIX_TIMESTAMP(`delete_time`) AS 'delete_time', " \
                                "`use_cash` * 100 AS 'cash', `use_card` * 100 AS 'card', `master_card` AS 'master', " \
                                "`remain_card` * 100 AS 'remain_card', `card_num`, d_list.`type` AS 'type' " \
                                "FROM gl_delete_self_state AS delete_self " \
                                "INNER JOIN gl_device_list AS d_list " \
                                "ON delete_self.device_addr = d_list.`addr`" \
                                "WHERE d_list.`type` = %s AND d_list.`addr` = %s"
                        curs.execute(query, (type, addr))
                        res = curs.fetchall()

                    elif type == str(gls_config.READER):
                        query = "SELECT delete_reader.`no`, UNIX_TIMESTAMP(`start_time`) AS 'start_time', " \
                                "UNIX_TIMESTAMP(`end_time`) AS 'end_time', " \
                                "UNIX_TIMESTAMP(`delete_time`) AS 'delete_time', " \
                                "`reader_cash` * 100 AS 'cash', `reader_card` * 100 AS 'card', `master_card` AS 'master', " \
                                "`remain_card` * 100 AS 'remain_card', `card_num`, d_list.`type` AS 'type' " \
                                "FROM gl_delete_reader_state AS delete_reader " \
                                "INNER JOIN gl_device_list AS d_list " \
                                "ON delete_reader.device_addr = d_list.`addr`" \
                                "WHERE d_list.`type` = %s AND d_list.`addr` = %s"
                        curs.execute(query, (type, addr))
                        res = curs.fetchall()
                    ...
                    for row in res:
                        ...
                        # 사용내역
                        if type == str(gls_config.SELF):  # str(row['type'])
                            temp_dict['enable_type'] = self.get_self_detail(row['no'], 'self')
                        elif type == str(gls_config.AIR):
                            temp_dict['enable_type'] = '진공'
                        elif type == str(gls_config.MATE):
                            temp_dict['enable_type'] = '매트'
                        elif type == str(gls_config.READER):
                            temp_dict['enable_type'] = '리더'
                        elif type == str(gls_config.GARAGE):
                            temp_dict['enable_type'] = self.get_self_detail(row['no'], 'garage')
                        ...
                        if row['card_num']:
                            temp_dict['card_num'] = row['card_num']
                        if row['delete_time']:
                            temp_dict['delete_time'] = row['delete_time']

                        dic_delete_data_select.append(temp_dict)

                # 충전장비
                elif type == str(gls_config.CHARGER) or type == str(gls_config.TOUCH) or type == str(gls_config.KIOSK):
                    query = "SELECT UNIX_TIMESTAMP(`input_date`) AS 'input_date'," \
                            "UNIX_TIMESTAMP(`delete_date`) AS 'delete_date', `kind`, " \
                            "`exhaust_money` * 100 AS 'card_price', " \
                            "`current_money` * 100 AS 'money', `current_bonus` * 100 AS 'bonus', " \
                            "`current_credit_money` * 100 AS 'credit_money', " \
                            "`current_charge` * 100 AS 'charge', `total_money` * 100 AS 'remain_card', " \
                            "`card_num` FROM gl_delete_charger_state AS charger " \
                            "INNER JOIN gl_device_list AS d_list " \
                            "ON charger.device_no = d_list.`no`" \
                            "WHERE d_list.`type` = %s AND d_list.`addr` = %s"
                    curs.execute(query, (type, addr))
                    ...
                    for row in res:
                        ...
                        if row['input_date']:
                            temp_dict['input_date'] = row['input_date']
                        if row['delete_date']:
                            temp_dict['delete_date'] = row['delete_date']
                        if row['card_num']:
                            temp_dict['card_num'] = row['card_num']
            ...
            print("From get_delete_data_select except : ", e)
        finally:
            conn.close()
            self.DELETE_STATE = False

        return dic_delete_data_select

    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    51. 장비 세부이력 - 선택 데이터 삭제 (세차장비)
    포스로부터 선택된 장비타입, 장비주소, 종료일, 현금/카드사용액, 카드잔액, 카드번호를 넘겨 받아
    선택된 데이터를 검색 후 삭제 테이블에 추가 후 해당 장비의 데이터 이력을 삭제한다. 
    # 실제 데이터는 완전삭제가 아닌 삭제 테이블에 삽입 후 이력에서 삭제  
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def select_device_data_delete(self, args):
        print(args)
        ...
        # 삭제 시간추가
        delete_time = datetime.today().strftime('%Y-%m-%d %H:%M:%S')
        ...
        try:
            ...
            with self.db_connect.cursor(pymysql.cursors.DictCursor) as curs:
                # 세차장비
                if (device_type == str(gls_config.SELF) or device_type == str(gls_config.AIR) or device_type == str(gls_config.MATE)
                        or device_type == str(gls_config.READER) or device_type == str(gls_config.GARAGE)):

                    # 선택 데이터 추출
                    if device_type == str(gls_config.SELF):
                        self_select_query = "SELECT t_self.`no` AS 'device_no', t_list.`type` AS 'device_type', `device_addr`, " \
                                            "UNIX_TIMESTAMP(`start_time`) AS 'start_time', " \
                                            "UNIX_TIMESTAMP(`end_time`) AS 'end_time', " \
                                            "`use_cash` * 100 AS 'cash', `use_card` * 100 AS 'card', `remain_card` * 100 AS 'remain_card', " \
                                            "`master_card` * 100 AS 'master', `card_num`, " \
                                            "`self_time`, `under_time`, `foam_time`, `coating_time` " \
                                            "FROM gl_self_state AS t_self " \
                                            "INNER join gl_device_list AS t_list ON t_self.device_addr = t_list.addr " \
                                            "WHERE t_list.`type` = %s AND t_self.`device_addr` = %s " \
                                            "AND start_time = %s AND end_time = %s " \
                                            "AND `use_cash` = %s AND `use_card` = %s AND `remain_card` = %s " \
                                            "AND `card_num` = %s"
                        curs.execute(self_select_query, (device_type, addr, start_time, end_time, cash, card, remain_card, card_num))
                        rows = curs.fetchone()

                        print(rows)
                        # 셀프 내역저장
                        device_no = rows['device_no']
                        self_time = rows['self_time']
                        under_time = rows['under_time']
                        foam_time = rows['foam_time']
                        coating_time = rows['coating_time']
                        master_card = str(int(rows['master']) // 100).rjust(4, "0")

                        # 삭제 테이블 이력추가
                        query_delete_self_state_insert = "INSERT INTO gl_delete_self_state " \
                                                         "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                        curs.execute(query_delete_self_state_insert,
                                     (device_no, addr, card_num, self_time, under_time, foam_time, coating_time, cash, card,
                                      remain_card, master_card, start_time, end_time, delete_time))

                        # 선택 이력 삭제
                        query_self_state_delete = "DELETE FROM gl_self_state " \
                                                  "WHERE `device_addr` = %s " \
                                                  "AND start_time = %s AND end_time = %s " \
                                                  "AND `use_cash` = %s AND `use_card` = %s AND `remain_card` = %s " \
                                                  "AND `master_card` = %s AND `card_num` = %s"
                        curs.execute(query_self_state_delete, (addr, start_time, end_time, cash, card, remain_card, master_card, card_num))
                        ...
                        reader_select_query = "SELECT t_self.`no`, t_list.`type` AS 'device_type', `device_addr`, " \
                                              ...
                                              "`master_card` * 100 AS 'master' FROM gl_reader_state AS t_self " \
                                              "INNER join gl_device_list AS t_list ON t_self.device_addr = t_list.addr " \
                                              "WHERE t_list.`type` = %s AND t_self.`device_addr` = %s " \
                                              "AND start_time = %s AND end_time = %s " \
                                              "AND `reader_cash` = %s AND `reader_card` = %s AND `remain_card` = %s AND `card_num` = %s"
                        curs.execute(reader_select_query, (device_type, addr, start_time, end_time, cash, card, remain_card, card_num))
                        rows = curs.fetchone()
                        ...
                        # 삭제 테이블 이력추가
                        query_delete_reader_state_insert = "INSERT INTO gl_delete_reader_state " \
                        ...
                        # 선택 이력 삭제
                        query_reader_state_delete = "DELETE FROM gl_reader_state " \
                                                    "WHERE `device_addr` = %s " \
                                                    "AND start_time = %s AND end_time = %s " \
                                                    "AND `reader_cash` = %s AND `reader_card` = %s AND `remain_card` = %s " \
                                                    "AND `master_card` = %s AND `card_num` = %s"
                        curs.execute(query_reader_state_delete, (addr, start_time, end_time, cash, card, remain_card, master_card, card_num))

                self.db_connect.commit()
                result = 1  # 반환값 : 성공
        except Exception as e:
            print("From select_device_data_delete Exception : " + str(e))
            result = 0  # 반환값 : 실패
        finally:
            self.closeConnectDB()
        return result
    ...
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    52. 장비 세부이력 - 선택 데이터 삭제 (충전장비)
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def select_charger_data_delete(self, args):
        # print(args)
        device_type = str(args['device_type'])
        device_addr = str(args['device_addr'])
        input_date = str(self.setStringDateToTimeStamp(args['input_date']))
        current_money = str(int(args['current_money']) // 100).rjust(4, '0')
        current_bonus = str(int(args['current_bonus']) // 100).rjust(5, '0')
        current_charge = str(int(args['current_charge']) // 100).rjust(4, '0')
        total_money = str(int(args['total_money']) // 100).rjust(5, '0')
        card_num = str(args['card_num'])

        input_date = self.setTimeStampToString(input_date)

        # 삭제 시간추가
        delete_date = datetime.today().strftime('%Y-%m-%d %H:%M:%S')

        try:
            self.openConnectDB()
            # curs = self.db_connect.cursor(pymysql.cursors.DictCursor)
            with self.db_connect.cursor(pymysql.cursors.DictCursor) as curs:
                if device_type == str(gls_config.CHARGER) or device_type == str(gls_config.TOUCH) or device_type == str(gls_config.KIOSK):

                    # 선택 데이터 추출
                    query_charger_select = "SELECT t_charger.`no` AS 'no', t_list.`no` AS 'device_no', " \
                                    "UNIX_TIMESTAMP(`input_date`) AS 'input_date', `kind`, `exhaust_money`*100 AS 'exhaust_money', " \
                                    "`current_money` * 100 AS 'current_money', " \
                                    "`current_bonus` * 100 AS 'current_bonus', `current_charge` * 100 AS 'current_charge', " \
                                    "`total_money` * 100 AS 'total_money', `card_num`, " \
                                    "charger_type, sales_type, current_credit_money, credit_no " \
                                    "FROM gl_charger_state AS t_charger " \
                                    "INNER JOIN gl_device_list AS t_list ON t_charger.device_no = t_list.`no` " \
                                    "WHERE input_date = %s AND `current_money` = %s AND `current_bonus` = %s " \
                                    "AND `current_charge` = %s AND `total_money` = %s AND `card_num` = %s"
                    curs.execute(query_charger_select, (
                    input_date, current_money, current_bonus, current_charge, total_money, card_num))
                    rows = curs.fetchone()
                    print(rows)
                    ...
                    device_no = rows['device_no']
                    kind = rows['kind']
                    charger_type = rows['charger_type']
                    sales_type = rows['sales_type']
                    current_credit_money = rows['current_credit_money']
                    credit_no = rows['credit_no']

                    # 소수점 절삭
                    exhaust_money = int(rows['exhaust_money'])
                    current_money = int(rows['current_money'])
                    current_bonus = int(rows['current_bonus'])
                    current_charge = int(rows['current_charge'])
                    total_money = int(rows['total_money'])

                    # 삭제 테이블 이력추가
                    exhaust_money = str(exhaust_money // 100).rjust(5, '0')
                    current_money = str(current_money // 100).rjust(4, '0')
                    current_bonus = str(current_bonus // 100).rjust(5, '0')
                    current_charge = str(current_charge // 100).rjust(4, '0')
                    total_money = str(total_money // 100).rjust(5, '0')

                    query_insert_deletestate = "INSERT INTO gl_delete_charger_state " \
                                         "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                    curs.execute(query_insert_deletestate, (no, device_no, kind, exhaust_money, charger_type, sales_type,
                                                      current_money, current_credit_money, current_bonus, current_charge
                                                      ,total_money, card_num, credit_no, input_date, delete_date))

                    # 이력 삭제
                    query_delete_state = "DELETE FROM gl_charger_state " \
                                         "WHERE input_date = %s " \
                                         "AND `current_money` = %s AND `current_bonus` = %s AND `current_charge` = %s " \
                                         "AND `total_money` = %s AND `card_num` = %s"
                    curs.execute(query_delete_state,
                                 (input_date, current_money, current_bonus, current_charge, total_money, card_num))
            ...
            print("From select_charger_data_delete Exception : " + str(e))
            result = 0  # 반환값 : 실패
        finally:
            self.closeConnectDB()
    ...
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    53. 삭제이력 - 선택 데이터 복원 (세차장비)
    포스로부터 장비타입, 장비주소, 시작일, 종료일, 삭제일, 현금/카드사용액, 카드잔액, 카드번호를 넘겨받아
        ...
        try:
            ...
            with self.db_connect.cursor(pymysql.cursors.DictCursor) as curs:
                ...
                if (device_type == str(gls_config.SELF) or device_type == str(gls_config.AIR) or device_type == str(gls_config.MATE)
                    ...
                    # 선택 삭제데이터 추출
                    if device_type == str(gls_config.SELF):
                        query_deleteself_select = "SELECT t_self.`no` AS 'device_no', t_list.`type` AS 'device_type', `device_addr`, " \
                                            "UNIX_TIMESTAMP(`start_time`) AS 'start_time', " \
                                            "UNIX_TIMESTAMP(`end_time`) AS 'end_time', " \
                                            "UNIX_TIMESTAMP(`delete_time`) AS 'delete_time', " \
                                            "`use_cash` * 100 AS 'cash', `use_card` * 100 AS 'card', `remain_card` * 100 AS 'remain_card', " \
                                            "`master_card` * 100 AS 'master', `card_num`, " \
                                            "`self_time`, `under_time`, `foam_time`, `coating_time` " \
                                            "FROM gl_delete_self_state AS t_self " \
                                            "INNER join gl_device_list AS t_list ON t_self.device_addr = t_list.addr " \
                                            "WHERE t_list.`type` = %s AND t_self.`device_addr` = %s " \
                                            "AND start_time = %s AND end_time = %s AND delete_time = %s " \
                                            "AND `use_cash` = %s AND `use_card` = %s AND `remain_card` = %s AND `card_num` = %s"
                        curs.execute(query_deleteself_select, (
                        ...
                        # 테이블 복원이력 추가
                        query_restore_self_state_insert = "INSERT INTO gl_self_state " \
                                                         "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                        curs.execute(query_restore_self_state_insert,
                                     (device_no, addr, card_num, self_time, under_time, foam_time, coating_time, cash, card,
                                      remain_card, master_card, start_time, end_time))

                        # 선택한 삭제이력 삭제
                        query_self_state_delete = "DELETE FROM gl_delete_self_state " \
                                                  "WHERE `device_addr` = %s " \
                                                  "AND start_time = %s AND end_time = %s AND delete_time = %s " \
                                                  "AND `use_cash` = %s AND `use_card` = %s AND `remain_card` = %s " \
                                                  "AND `card_num` = %s"
                        curs.execute(query_self_state_delete,
                                     (addr, start_time, end_time, delete_time, cash, card, remain_card, card_num))
            ...
            print("From restore_select_device_data Exception : " + str(e))
            result = 0  # 반환값 : 실패
        finally:
            self.closeConnectDB()
    ...
    def restore_select_charger_data(self, args):
        ...
        device_addr = str(args['device_addr'])
        input_date = str(self.setStringDateToTimeStamp(args['input_date']))
        delete_date = str(self.setStringDateToTimeStamp(args['delete_date']))
        current_money = str(int(args['current_money']) // 100).rjust(4, '0')
        current_bonus = str(int(args['current_bonus']) // 100).rjust(5, '0')
        current_charge = str(int(args['current_charge']) // 100).rjust(4, '0')
        total_money = str(int(args['total_money']) // 100).rjust(5, '0')
        ...
        # 타임스탬프 변환처리
        input_date = self.setTimeStampToString(input_date)
        delete_date = self.setTimeStampToString(delete_date)
        # TimeStamp -> Str date
        # input_date = int(input_date)
        # input_date = datetime.fromtimestamp(input_date).strftime('%Y-%m-%d %H:%M:%S')
        # delete_date = int(delete_date)
        # delete_date = datetime.fromtimestamp(delete_date).strftime('%Y-%m-%d %H:%M:%S')
        ...
        try:
            ...
            with self.db_connect.cursor(pymysql.cursors.DictCursor) as curs:
                if device_type == str(gls_config.CHARGER) or device_type == str(gls_config.TOUCH) or device_type == str(gls_config.KIOSK):
                    ...
                    # 선택 복원데이터 추출
                    query_charger_select = "SELECT t_charger.`no` AS 'no', t_list.`no` AS 'device_no', " \
                                           "UNIX_TIMESTAMP(`input_date`) AS 'input_date', UNIX_TIMESTAMP(`delete_date`) AS 'delete_date', " \
                                           "`kind`, `exhaust_money`*100 AS 'exhaust_money', `current_money` * 100 AS 'current_money', " \
                                           "`current_bonus` * 100 AS 'current_bonus', `current_charge` * 100 AS 'current_charge', " \
                                           "`total_money` * 100 AS 'total_money', `card_num`, " \
                                           "charger_type, sales_type, current_credit_money, credit_no " \
                                           "FROM gl_delete_charger_state AS t_charger " \
                                           "INNER JOIN gl_device_list AS t_list ON t_charger.device_no = t_list.`no` " \
                                           "WHERE input_date = %s AND delete_date = %s AND `current_money` = %s AND `current_bonus` = %s " \
                                           "AND `current_charge` = %s AND `total_money` = %s AND `card_num` = %s"
                    curs.execute(query_charger_select,
                         (input_date, delete_date, current_money, current_bonus, current_charge, total_money, card_num))
                    rows = curs.fetchone()
                    print(rows)
                    ...
                    query_insert_state = "INSERT INTO gl_charger_state " \
                                         "VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)"
                    curs.execute(query_insert_state,
                                 (no, device_no, kind, exhaust_money, charger_type, sales_type,
                                  current_money, current_credit_money, current_bonus, current_charge
                                  , total_money, card_num, credit_no, input_date))

                    # 삭제 이력 삭제
                    query_delete_state = "DELETE FROM gl_delete_charger_state " \
                                         "WHERE input_date = %s AND delete_date = %s " \
                                         "AND `current_money` = %s AND `current_bonus` = %s AND `current_charge` = %s " \
                                         "AND `total_money` = %s AND `card_num` = %s"
                    curs.execute(query_delete_state,
                                 (input_date, delete_date, current_money, current_bonus, current_charge, total_money, card_num))
            ...
            print("From restore_select_charger_data Exception : " + str(error))
            result = 0  # 반환값 : 실패
        finally:
            self.closeConnectDB()
    ...
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    55. 삭제이력 - 선택 데이터 완전삭제 (세차장비)
    포스로부터 장비타입, 장비주소, 시작일, 종료일, 삭제일, 현금/카드사용액, 카드잔액, 카드번호를 넘겨받아
    선택 데이터를 삭제 이력테이블에서 완전 삭제한다.  
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def complete_select_device_data_delete(self, args):
        # print(args)
        device_type = str(args['device_type'])
        addr = str(args['device_addr'])
        start_time = str(self.setStringDateToTimeStamp(args['start_time']))
    ...
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    56. 삭제이력 - 선택 데이터 완전삭제 (충전장비)
    """""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""""
    def complete_select_charger_data_delete(self, args):
        # print(args)
        device_type = str(args['device_type'])
        device_addr = str(args['device_addr'])
        input_date = str(self.setStringDateToTimeStamp(args['input_date']))
        ...
        try:
            ...
            print("From complete_select_charger_data_delete Exception : " + str(error))
            result = 0  # 반환값 : 실패
        finally:
            self.closeConnectDB()
    ...
    # date str -> timestamp
    def setStringDateToTimeStamp(self, str_date):
        timestamp = time.mktime(datetime.strptime(str_date, '%Y-%m-%d %H:%M:%S').timetuple())
        timestamp = int(timestamp)

        return timestamp

    # timestamp -> date str
    def setTimeStampToString(self, timestamp):
        timestamp = int(timestamp)
    ...
    def openConnectDB(self):
        try:
            self.db_connect = pymysql.connect(host=gls_config.MYSQL_HOST, user=gls_config.MYSQL_USER,
                                   password=gls_config.MYSQL_PWD, charset=gls_config.MYSQL_SET, db=gls_config.MYSQL_DB)
            # print("DB Open")
        except Exception as error:
            print("open ConnectDB Error : " + str(error))
    ...
    def closeConnectDB(self):
        try:
            ...
            print("close ConnectDB Error : " + str(error))
