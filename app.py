...
try :
    ...
    with conn.cursor():
        ...
        dv.set_time()
        dv.main_thread(1)         # 초기에 루프 한번돌고 그 다음부터는 Device.py main_thread 에서 실행
        # dv.thread_monitor(10)   # 초기 버전 : 주석 풀고 dv.thread_monitor(600)
...
class StartThread(Resource):
    ...
    def post(self):
        ...
        # print("디버깅")
        # print("USE           : ", dv.USE)
        # print("USE_EACH      : ", dv.USE)
        # print("TIME_USE      : ", dv.TIME_USE)
        # print("TIME_USE_EACH : ", dv.TIME_USE_EACH)
        # print("FLAG_MAIN     : ", dv.FLAG_MAIN)
        # print("FLAG_STATE    : ", dv.FLAG_STATE)
        pass
...
# 장비세부이력 - 삭제 데이터 조회
class GetDeletedataSelect(Resource):
    def post(self):
        parser = reqparse.RequestParser()  # 파서 객체생성
        parser.add_argument('device_type', type=str)
        parser.add_argument('device_addr', type=str)
        args = parser.parse_args()
        ...
        result = ps.get_delete_data_select(type, addr)
        return {'result': result}


# 장비세부이력 - 세차장비 선택 데이터 삭제
class SelectDeviceDataDelete(Resource):
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('device_type', type=str)
        parser.add_argument('device_addr', type=str)
        ...
        result = ps.select_device_data_delete(args)
        return {'result': result}


# 장비세부이력 - 충전장비 선택 데이터 삭제
class SelectChargerDataDelete(Resource):
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('device_type', type=str)
        parser.add_argument('device_addr', type=str)
        parser.add_argument('input_date', type=str)
        parser.add_argument('current_money', type=str)
        parser.add_argument('current_bonus', type=str)
        parser.add_argument('current_charge', type=str)
        parser.add_argument('total_money', type=str)
        parser.add_argument('card_num', type=str)

        args = parser.parse_args()

        result = ps.select_charger_data_delete(args)
        return {'result': result}


# 삭제 이력 - 선택 데이터 복원 (세차장비)
class RestoreSelectDeviceData(Resource):
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('device_type', type=str)
        parser.add_argument('device_addr', type=str)
        parser.add_argument('start_time', type=str)
        parser.add_argument('end_time', type=str)
        parser.add_argument('delete_time', type=str)
        parser.add_argument('cash', type=str)
        parser.add_argument('card', type=str)
        parser.add_argument('remain_card', type=str)
        parser.add_argument('card_num', type=str)
        ...
        result = ps.restore_select_device_data(args)
        return {'result': result}


# 삭제이력 - 선택 데이터복원 (충전장비)
class RestoreSelectChargerData(Resource):
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('device_type', type=str)
        parser.add_argument('device_addr', type=str)
        parser.add_argument('input_date', type=str)
        parser.add_argument('delete_date', type=str)
        parser.add_argument('current_money', type=str)
        parser.add_argument('current_bonus', type=str)
        parser.add_argument('current_charge', type=str)
        parser.add_argument('total_money', type=str)
        ...
        result = ps.restore_select_charger_data(args)
        return {'result': result}


# 삭제이력 - 선택 데이터 완전삭제 (세차장비)
class CompleteDeleteSelectDeviceData(Resource):
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('device_type', type=str)
        parser.add_argument('device_addr', type=str)
        ...
        result = ps.complete_select_device_data_delete(args)
        return {'result': result}


# 삭제이력 - 선택 데이터 완전삭제 (충전장비)
class CompleteDeleteSelectChargeData(Resource):
    def post(self):
        parser = reqparse.RequestParser()
        parser.add_argument('device_type', type=str)
        parser.add_argument('device_addr', type=str)
        ...
        result = ps.complete_select_charger_data_delete(args)
        return {'result': result}
...
# TODO : 20/05.22 대진 업데이트 내용 - 장비 세부 이력 조회 : 삭제 기능
api.add_resource(GetDeletedataSelect, '/get_delete_data_select')            # 삭제 데이터 조회

api.add_resource(SelectDeviceDataDelete, '/select_device_data_delete')      # 선택 데이터 삭제 (세차장비)
api.add_resource(SelectChargerDataDelete, '/select_charger_data_delete')    # 선택 데이터 삭제 (충전장비)

api.add_resource(RestoreSelectDeviceData, '/restore_select_device_data')    # 선택 데이터 복원 (세차장비)
api.add_resource(RestoreSelectChargerData, '/restore_select_charger_data')  # 선택 데이터 복원 (충전장비)

api.add_resource(CompleteDeleteSelectDeviceData, '/complete_select_device_data_delete')   # 선택 데이터 완전삭제 (세차장비)
api.add_resource(CompleteDeleteSelectChargeData, '/complete_select_charger_data_delete')  # 선택 데이터 완전삭제 (충전장비)
# TODO : 대진 업데이트 내용 끝
