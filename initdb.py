...
# 리더기 초기화 테이블 변경됨
# TODO : 삭제이력 테이블 초기화하기 (셀프, 리더기, 충전기)

try:
    with conn.cursor():

        try:
            print("reset gl_delete_self_state...")
            acd = "DELETE FROM gl_delete_self_state"
            curs.execute(acd)
            conn.commit()
        except Exception as e:
            print("From reset_gl_delete_self_state except :", e)
        finally:
            print("completed ! ")

        try:
            print("reset gl_delete_reader_state...")
            acd = "DELETE FROM gl_delete_reader_state"
            curs.execute(acd)
            conn.commit()
        except Exception as e:
            print("From reset_gl_delete_reader_state except :", e)
        finally:
            print("completed ! ")


        try:
            print("reset gl_delete_charger_state...")
            acd = "DELETE FROM gl_delete_charger_state"
            curs.execute(acd)
            conn.commit()
        except Exception as e:
            print("From reset_gl_delete_charger_state except :", e)
        finally:
            print("completed ! ")
